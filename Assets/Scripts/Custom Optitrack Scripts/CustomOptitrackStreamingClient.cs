...
//Note that global definitions are in the original Optitrack Client Script!
public class CustomOptitrackStreamingClient : MonoBehaviour
{
    ...
    public static CustomOptitrackStreamingClient FindDefaultClient()
    {
        CustomOptitrackStreamingClient[] allClients = FindObjectsOfType<CustomOptitrackStreamingClient>();

        if (allClients.Length == 0)
        {
            Debug.LogError("Unable to locate any " + typeof(CustomOptitrackStreamingClient).FullName + " components.");
            return null;
        }
        else if (allClients.Length > 1)
        {
            Debug.LogWarning("Multiple " + typeof(CustomOptitrackStreamingClient).FullName + " components found in scene; defaulting to first available.");
        }
